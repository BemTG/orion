...
class Modulo(RunAll):
    ...
    def fp8x23():
        def fp8x23_float_mixed_signs():
            ...
            name = "fp8x23_float_mixed_signs"
            make_test([_x, _y], _z, f"input_0.modulo(@input_1 , @input_2 , Option::Some({fmod_} )", name)

        def fp8x23_float_positive_signs():
            x = np.random.uniform(0, 10, (6)).astype(np.float32)
            y = np.random.uniform(0, 10, (6)).astype(np.float32)
            z = np.fmod(x, y)
            fmod_ = True
            ...
            name = "fp8x23_float_positive_signs"
            make_test([_x, _y], _z, f"input_0.modulo(@input_1 , @input_2 , Option::Some({fmod_} )", name)

        def fp8x23_float_negative_signs():
            x = np.random.uniform(0, -10, (6)).astype(np.float32)
            y = np.random.uniform(0, -10, (6)).astype(np.float32)
            z = np.fmod(x, y)
            fmod_ = True
            ...
            name = "fp8x23_float_negative_signs"
            make_test([_x, _y], _z, f"input_0.modulo(@input_1 , @input_2 , Option::Some({fmod_} )", name)


        def fp8x23_int_mixed_signs():
            ...
            name = "fp8x23_int_mixed_signs"
            make_test([_x, _y], _z, f"input_0.modulo(@input_1 , @input_2 , Option::Some({fmod_} )", name)

        def fp8x23_int_positive_signs():
            x = np.random.uniform(0, 10, (6)).astype(np.int32)
            y = np.random.uniform(0, 10, (6)).astype(np.int32)
            z = np.mod(x, y)
            fmod_ = False
            ...
            name = "fp8x23_int_positive_signs"
            make_test([_x, _y], _z, f"input_0.modulo(@input_1 , @input_2 , Option::Some({fmod_} )", name)

        def fp8x23_int_negative_signs():
            x = np.random.uniform(0, -10, (6)).astype(np.int32)
            y = np.random.uniform(0, -10, (6)).astype(np.int32)
            z = np.mod(x, y)
            fmod_ = False
            ...
            name = "fp8x23_int_negative_signs"
            make_test([_x, _y], _z, f"input_0.modulo(@input_1 , @input_2 , Option::Some({fmod_} )", name)
        ...
        def fp8x23_int_mod_broadcast():
            ...
            name = "fp8x23_mod_broadcast"
            make_test([_x, _y], _z, f"input_0.modulo(@input_1 , @input_2 , Option::None(())", name)
        ...
        fp8x23_float_mixed_signs()
        fp8x23_float_positive_signs()
        fp8x23_float_negative_signs()
        fp8x23_int_mixed_signs()
        fp8x23_int_positive_signs()
        fp8x23_int_negative_signs()
        fp8x23_int_mod_broadcast()
        fp8x23_float_mod_broadcast()


    def fp16x16():
        def fp16x16_float_mixed_signs():
            x = np.random.uniform(-10, 10, (6)).astype(np.float32)
            y = np.random.uniform(-10, 10, (6)).astype(np.float32)
            z = np.fmod(x, y)
            fmod_ = True
            ...
            name = "fp16x16_float_mixed_signs"
            make_test([_x, _y], _z, f"input_0.modulo(@input_1 , @input_2 , Option::Some({fmod_} )", name)

        def fp16x16_float_positive_signs():
            x = np.random.uniform(0, 10, (6)).astype(np.float32)
            y = np.random.uniform(0, 10, (6)).astype(np.float32)
            z = np.fmod(x, y)
            fmod_ = True
            ...
            name = "fp16x16_float_positive_signs"
            make_test([_x, _y], _z, f"input_0.modulo(@input_1 , @input_2 , Option::Some({fmod_} )", name)

        def fp16x16_float_negative_signs():
            ...
            name = "fp16x16_float_negative_signs"
            make_test([_x, _y], _z, f"input_0.modulo(@input_1 , @input_2 , Option::Some({fmod_} )", name)

        def fp16x16_int_mixed_signs():
            x = np.random.uniform(-10, 10, (6)).astype(np.int32)
            y = np.random.uniform(-10, 10, (6)).astype(np.int32)
            z = np.mod(x, y)
            fmod_ = False
            ...
            name = "fp16x16_int_mixed_signs"
            make_test([_x, _y], _z, f"input_0.modulo(@input_1 , @input_2 , Option::Some({fmod_} )", name)

        def fp16x16_int_positive_signs():
            x = np.random.uniform(0, 10, (6)).astype(np.int32)
            y = np.random.uniform(0, 10, (6)).astype(np.int32)
            z = np.mod(x, y)
            fmod_ = False
            ...
            name = "fp16x16_int_positive_signs"
            make_test([_x, _y], _z, f"input_0.modulo(@input_1 , @input_2 , Option::Some({fmod_} )", name)

        def fp16x16_int_negative_signs():
        ...
        fp16x16_float_mixed_signs()
        fp16x16_float_positive_signs()
        fp16x16_float_negative_signs()
        fp16x16_int_mixed_signs()
        fp16x16_int_positive_signs()
        fp16x16_int_negative_signs()
